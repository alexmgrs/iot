    ...
    const handleDeleteMember = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:3001/place/member', {
                ...
                body: JSON.stringify({
                    place_id: deleteMemberData.place_id,
                    username: deleteMemberData.username
                })
            });
            if (!response.ok) {
                throw new Error('Error deleting member from place');
            }
            alert('Member deleted from place successfully.');
            setDeleteMemberData({ place_id: '', username: '' }); // Réinitialiser les données de suppression de membre après la suppression réussie
        } catch (error) {
            console.error('Error deleting member from place:', error);
        }
    };
            ...
            <ul>
                {places.map((place, index) => (
                    <li key={index}>
                        ...
                        {/* Add button to show delete member form */}
                        <button onClick={() => handleDeleteMemberClick(place.id)}>Delete Member</button>
                        {/* Show form if the place ID matches the place ID in deleteMemberData */}
